...
function buildMarketMessage(itemData, activeTf, isGuildMember, clientUser) {
    ...
    const riskNote = "⚠️ **投資有風險，請保持獨立判斷，審慎評估風險，祝大家楓之股滿盆砵缽～**";
...
async function handleSystemCommand(interaction, client, isOwner, hasAdminRole, hasAdminPerm, isGuildMember) {
    ...
    if (['貼圖', '新增貼圖', '刪除貼圖'].includes(cmd)) {
        ...
        if (cmd === '貼圖') {
            if (!stickers || stickers.length === 0) return interaction.reply({ content: '❌ 目前圖庫空空如也，請管理員使用 `/新增貼圖` 來建立吧！', flags: MessageFlags.Ephemeral });
            try {
                ...
                const row = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId('select_sticker').setPlaceholder('請選擇要發送的貼圖...').addOptions(options));
    ...
    if (['解鎖權限', '發布小指南', '發布市場看板', '查詢目前公會成員', '查詢目前親友團', '同步更名', '清除資料', '清除訊息', '星光紅毯設定'].includes(cmd)) {
        if (cmd === '星光紅毯設定') {
            await interaction.deferReply({ flags: MessageFlags.Ephemeral });
            if (!interaction.member.premiumSince) return interaction.editReply('❌ 很抱歉，這個酷炫的功能是 **Server Booster** 專屬的特權喔！');
        ...
        if (cmd === '同步更名') {
            await interaction.deferReply({ flags: MessageFlags.Ephemeral });
            await interaction.editReply('⏳ 開始同步伺服器成員暱稱，這會需要幾十秒的時間，請稍候...');
            try {
...
module.exports = { handleSystemCommand };
